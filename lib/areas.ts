    ...
    loops: [
      {
        ...
        description:
          "Heads up before you plan around this one: most of this loop is currently closed by a forest order (see the closure notice up top), so confirm it has reopened before making the drive. When it's open, it's the three green-sticker roads that make this range worth it: East Camino Cielo (5N12.1) along the crest above town, the Romero–Camuesa spine (5N15.2) into the Camuesa OHV area, and Buckhorn (9N11.4) deep into the San Rafael backcountry. Treat it as a committing all-day epic, not a casual outing: long, remote, and lightly traveled, so carry water, fuel, and a way to call for help. The Camuesa side also closes after rain (the wet-season gate closures, roughly November–April), and the access roads between these stretches are frequently gated, so they don't always link cleanly. Bring an Adventure Pass. Open and dry, it's the best green-sticker riding in the area.",
